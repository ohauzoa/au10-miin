...
static int command(int input, int first, int last, char *cmd_exec)
{
    ...
    if (pid == 0) 
    {
        ...
        if (strcmp(args[0], "fb") == 0)
        {
            _fb2bmp(args);
        } 
        else if (strcmp(args[0], "ntp") == 0)
        {
            _ntp(args);
        } 
        else if (strcmp(args[0], "ds") == 0)
        {
            ds1307_clear();
        } 
...
{
    int thr_id;
    thr_id = pthread_create(&cli_thread, NULL, task_cli, "task cli");
}
